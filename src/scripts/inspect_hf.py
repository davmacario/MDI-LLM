...
def main(args):
    ...
    # TODO: download model from HF if not found locally

    # TODO: load the model "gradually" as done in weight conversion
    # Should use the '.bin' files in the model directory
    # >>>>>>
    ...
    # TODO: probably remove (count_transformer_blocks will not work)
    # >>>>>>
    ...
    os.makedirs(hf_keys_dir, exist_ok=True)
    hf_keys_file = hf_keys_dir / f"{model_name}_params_keys_lit.txt"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="""Inspect pretrained model checkpoints from Huggingface"""
    )
    parser.add_argument(
        "-s",
        "--save",
        action="store_true",
        help="save model parameter names to default location (src/tmp/...)",
    )
    parser.add_argument(
        "--model",
        type=str,
        ...
        model stored in (a) '.bin' file(s) and model_config.yaml files) or a Huggingface
        model (that will be downloaded)""",
    )
    parser.add_argument(
        "--ckpt-folder",
        type=str,
        ...
        model files will be placed at `<ckpt_folder>/<model>/`""",
    )
    parser.add_argument(
        "--device",
        type=str,
